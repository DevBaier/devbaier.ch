    ...
    stage('Build Docker Image on Portainer') {
      steps {
        script {
          // Build the image
              def repoURL = """
                https://portainer.devbaier.ch/api/endpoints/1/docker/build?t=devbaier:latest&remote=https://raw.githubusercontent.com/DevBaier/test/main/Dockerfile&nocache=true
              """
