    ...
    stage('Build Docker Image on Portainer') {
      steps {
        script {
          // Build the image
              def repoURL = """
                https://portainer.devbaier.ch/api/endpoints/1/docker/build?t=devbaierWeb:latest&remote=https://raw.githubusercontent.com/DevBaier/devbaier.ch/main/Dockerfile&nocache=true
              """
